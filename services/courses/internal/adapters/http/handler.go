...
func NewHandler(courses *application.Courses) *Handler {
...
func (h *Handler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	response := courseDetailsResponse{
		...
		Theme:       course.Course.Theme,
		Description: course.Course.Description,
		Price:       course.Course.Price,
		AuthorID:    course.Course.AuthorID,
		Status:      course.Course.Status,
		Sections:    make([]sectionDetailsResponse, 0, len(course.Sections)),
	}
	...
	for _, section := range course.Sections {
		sectionResponse := sectionDetailsResponse{
			ID:          section.Section.ID,
			Title:       section.Section.Title,
			Description: section.Section.Description,
			Position:    section.Section.Position,
			Pages:       make([]pageResponse, 0, len(section.Pages)),
		}

		for _, page := range section.Pages {
			sectionResponse.Pages = append(sectionResponse.Pages, pageResponse{
				ID:       page.ID,
				Title:    page.Title,
				Type:     page.Type,
		...
		response.Sections = append(response.Sections, sectionResponse)
	}
...
func (h *Handler) Enroll(w http.ResponseWriter, r *http.Request) {
	...
	err = h.courses.Enroll(r.Context(), userID, courseID)
	if err != nil {
		switch {
