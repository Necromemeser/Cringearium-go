...
type courseDetailsResponse struct {
	...
	AuthorID    *int64                   `json:"author_id,omitempty"`
	Status      domain.CourseStatus      `json:"status"`
	Sections    []sectionDetailsResponse `json:"sections"`
}
...
func (h *Handler) GetAll(w http.ResponseWriter, r *http.Request) {
	courses, err := h.courses.GetAll(r.Context())
	if err != nil {
...
func (h *Handler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	if course.Course.ImageID != nil {
		response.ImageID = *course.Course.ImageID
	}
	for _, section := range course.Sections {
		sr := sectionDetailsResponse{ID: section.Section.ID, Title: section.Section.Title, Description: section.Section.Description, Position: section.Section.Position, Pages: make([]pageResponse, 0, len(section.Pages))}
		for _, page := range section.Pages {
			sr.Pages = append(sr.Pages, pageResponse{ID: page.ID, Title: page.Title, Type: page.Type, Content: page.Content, Position: page.Position})
		}
		response.Sections = append(response.Sections, sr)
	}
	writeJSON(w, http.StatusOK, response)
}
func (h *Handler) Enroll(w http.ResponseWriter, r *http.Request) {
	...
	if err = h.courses.Enroll(r.Context(), userID, courseID); err != nil {
		switch {
		case errors.Is(err, application.ErrCourseNotFound):
			http.Error(w, "course not found", http.StatusNotFound)
		case errors.Is(err, application.ErrCourseNotPublished):
			http.Error(w, "course is not published", http.StatusConflict)
		case errors.Is(err, application.ErrCourseNotFree):
			http.Error(w, "paid courses must be purchased", http.StatusConflict)
		case errors.Is(err, application.ErrAlreadyHasAccess):
...
func (h *Handler) GetAccess(w http.ResponseWriter, r *http.Request) {
	...
	hasAccess, err := h.courses.HasAccess(r.Context(), userID, courseID)
	if err != nil {
		if errors.Is(err, application.ErrCourseNotFound) {
			http.Error(w, "course not found", http.StatusNotFound)
			return
...
func (h *Handler) GetEnrolled(w http.ResponseWriter, r *http.Request) {
	...
	courses, err := h.courses.FindEnrolled(r.Context(), userID)
	if err != nil {
		http.Error(w, "internal server error", http.StatusInternalServerError)
		return
	}
...
func (h *Handler) GetProgress(w http.ResponseWriter, r *http.Request) {
	...
	pageIDs, err := h.courses.GetCompletedPages(r.Context(), userID, courseID)
	if err != nil {
		http.Error(w, "internal server error", http.StatusInternalServerError)
		return
	}
...
func (h *Handler) CompletePage(w http.ResponseWriter, r *http.Request) {
	...
	if err = h.courses.CompletePage(r.Context(), userID, pageID); err != nil {
		http.Error(w, "internal server error", http.StatusInternalServerError)
		return
	}
...
func toCourseResponse(course *domain.Course) courseResponse {
	response := courseResponse{ID: course.ID, Title: course.Title, Theme: course.Theme, Description: course.Description, Price: course.Price, AuthorID: course.AuthorID, Status: course.Status}
	if course.ImageID != nil {
