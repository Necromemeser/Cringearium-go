...
export default function TestPage({ pageId, token, onPassed }: TestPageProps) {
  ...
  if (loading) {
    return <div className="test-loading">Загружаем тест...</div>
  }
  ...
  return (
    <div className="test-container">
      <div className="test-meta">
        ...
        <strong>
          {locked
            ? `Результат: ${result?.score ?? 0}%`
            : `${answeredCount} / ${questionCount} отвечено`}
        </strong>
      </div>

      {locked && (
        <div className="test-completed-note">
          Тест уже пройден. Ответы доступны только для просмотра.
        </div>
      )}
      ...
      {error && (
        <div className="form-error test-form-error">
          {error}
        </div>
      )}
      ...
      {!locked && (
        <div className="test-submit">
          <div>
            <strong>
              {answeredCount} из {questionCount}
            </strong>
